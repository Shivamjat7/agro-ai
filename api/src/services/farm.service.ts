...
export const createFarm = async (userId: number, payload: any) => {
...
export const getFarmById = async (userId: string, farmId: string) => {
    const farm = await db.query.farms.findFirst({
        where: and(eq(farms.id, farmId), eq(farms.userId, userId), isNull(farms.deletedAt)),
    });
...
export const updateFarm = async (userId: string, farmId: string, payload: any) => {
    const farm = await getFarmById(userId, farmId);
